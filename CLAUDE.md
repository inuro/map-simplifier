...
- `package.json` の `version` を改良ごとに **マイナー番号** を上げる形で更新する（0.1.0 → 0.2.0 → 0.3.0 …）。書籍原稿向けの内輪ツールで SemVer を厳密に切る必然は薄いが、「今触っているビルドがどの改良を含むか」を識別するための目印として運用する。
- Vite の `define` で `package.json.version` を `__APP_VERSION__` に注入し、ヘッダの `#app-version` に `v0.x.y` 形式で表示する。ブラウザを更新すれば現在のバージョンが目視で分かる。
