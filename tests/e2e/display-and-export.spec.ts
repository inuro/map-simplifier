  ...
  await expect
    .poll(async () => {
      return await page.evaluate(() => {
        const popover = document.querySelector("#layer-visibility-popover");
        const header = document.querySelector("header");
        if (!(popover instanceof HTMLElement) || !(header instanceof HTMLElement)) {
          return { extendsBelowHeader: false, topInsidePopover: false };
        }
        ...
        return {
          extendsBelowHeader: pr.bottom > hr.bottom,
          topInsidePopover: !!topElement?.closest("#layer-visibility-popover"),
        };
      });
    })
    .toEqual({ extendsBelowHeader: true, topInsidePopover: true });
...
test("編集中は同じ整数ズーム帯にロックし、リセットで解除、表示はリアルタイム更新", async ({ page }) => {
  await page.goto("/");
  await page.waitForFunction(() => document.body.dataset.mapReady === "true", null, {
    timeout: 30_000,
  });
  ...
  await page.waitForFunction(
    () => {
      ...
      return (
        !!g.__mlMap &&
        Math.abs(g.__mlMap.getZoom() - 16.35) < 0.01 &&
        g.__mlMap.isStyleLoaded() &&
        g.__mlMap.areTilesLoaded()
      );
    },
  ...
  // ロック発動：同じ整数ズーム帯（16.0 <= z < 17.0）の範囲だけ許可する
  ...
  expect(afterLock.zoom).toBeCloseTo(16.35, 2);
  expect(afterLock.min).toBe(16);
  expect(afterLock.max).toBeCloseTo(16.999999, 6);
  ...
  // 同じ整数ズーム帯の中では表示倍率を微調整できる
  await page.evaluate(() => {
    const g = window as unknown as { __mlMap?: { setZoom(z: number): void } };
    g.__mlMap?.setZoom(16.8);
  });
  await expect
    .poll(async () => {
      return await page.evaluate(() => {
